...
|-------------------------|-----------|-------------|
| `brand_id`             | BIGINT     | Unique identifier for the brand |
...
The **Brands Table** serves as the authoritative record for all brands in the system, tracking not only metadata and descriptions but also real-time ingestion statistics and competitive relationships. The use of **JSONB fields** for metadata and demographics ensures flexibility in storing structured brand data, while **BYTEA blobs** for `followed_by_customer` and `analyzed_by_customer` enable efficient retrieval of relationships without complex joins.  Proper management of this table allows for dynamic insights into brand visibility, audience engagement, and market competition.

The `brand_id` is initially the Facebook Page ID of the brand.
...
<img src="https://billdonner.com/adology/S3-Storage-Zones.png" width=400>


| Zone         | Description |
|-------------|-------------|
| **Shared Zone**  | These are all the records that everyone uses. |
| **Customer A**   | These are Customer A's private records and AI harvestings. |
| **Customer B**   | These are all Customer B's private records and AI harvestings. |

As objects (videos, images, text, etc) are initially acquired they are copied into the S3 shared space and additional fields are recorded in the ADs Table for everyone's benefit.
...
{
  "brands": {
    "brand_id": { "type": "Number", "unique": true, "required": true },
